...
export default function ThemeToggle({ className = "" }: { className?: string }) {
  ...
  return (
    <button
      ...
      className={`focus-ring group relative flex h-10 w-10 items-center justify-center rounded-full border border-tc-border bg-tc-glass-bg text-tc-text-muted transition-colors duration-[var(--tc-dur)] hover:border-tc-primary hover:text-tc-primary ${className}`}
    >
      <Sun
        size={16}
